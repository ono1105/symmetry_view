...
class NativePyVistaViewer:
    ...
    def show(self) -> None:
        self.animated_atoms = viewer.add_animated_atoms(
            self.plotter,
            self.render_data,
    ...
    def update_start_markers(self) -> None:
        for item, actor in zip(self.animated_atoms, self.start_marker_actors):
            visible = item["atom"]["index"] in self.paths
            try:
...
def main() -> int:
    ...
    parser.add_argument(
        "--scope",
        choices=("all", "representative", "selected"),
        default="all",
