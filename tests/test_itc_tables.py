...
from crystal_viewer.itc_tables import (
    itc_coordinate_summaries,
    itc_operation_notation_summaries,
    load_itc_operation_data,
    load_itc_operation_notation_data,
)
...
class ItcTablesTest(unittest.TestCase):
    ...
    def test_operation_notation_table_contains_all_space_groups(self):
        data = load_itc_operation_notation_data()

        self.assertEqual(len(data["space_groups"]), 230)
        self.assertEqual(len(data["space_groups"]["225"]["descriptions"][0]["operations"]), 192)
    ...
    def test_batio3_conventional_cell_uses_pdf_operation_notation(self):
        payload = json.loads(Path("exports/json/batio3.json").read_text(encoding="utf-8"))
        converted = standardized_payload(payload, "conventional", require_distinct=True)

        self.assertIsNotNone(converted)
        assert converted is not None
        matches = itc_operation_notation_summaries(converted["render_data"])

        self.assertIn("g(", matches[9])
        self.assertIn("1/6", matches[9])
        self.assertIn("x + 1/2", matches[9])

    def test_halite_uses_pdf_operation_notation_for_all_operations(self):
        payload = json.loads(Path("exports/json/halite.json").read_text(encoding="utf-8"))
        render_data = payload["render_data"]

        matches = itc_operation_notation_summaries(render_data)
