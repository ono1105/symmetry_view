...
## 操作あてクイズ追加・パズルUI整備＋精査（Claude）（2026-07-08）

回転軸クイズに加え、パズルに「操作あてクイズ」を追加（クイズ選択→構造選択→プレイ）。1つの対称操作をアニメーション表示し、種類（回転／鏡映／反転／回映Sn／回反-n）と、次数を持つものは次数を答えさせる。判定は `crystal_viewer/game/operation_identify.py`（新規、テスト10件）、API は `/api/puzzle/operations`・`/check`。3Dは引き続き解析モードの `StaticStructureView` を再利用。

### 実装の要点と判断

- **回映/回反の次数**は行列周期でなく回転角から算出（回映は cos=(tr+1)/2、回反は符号反転）。分子は回映Sn、結晶は回反-n を提示（`source_kind` で選択肢を出し分け）。次数候補 {3,4,6}。
- **恒等と区別できない出題の排除**：全原子が自分自身に写る（動かない）操作は除外。例：平面分子の分子面鏡映。
- **見分けられない操作は除外でなく統合**：運動の種類（回転=弧／鏡映・反転=直線）と全原子の移動先が一致する操作は1問に統合し、当てはまる名前をすべて正解として受理（`answers` リスト）。CO2 の反転≡垂直鏡映は1問で反転・鏡映どちらも正解。平面分子の Cn≡Sn も統合。全除外にすると CO2 が0問になり「対称性が無い」と誤読されるため、統合方式を採用。
- **無限回回転**：分子の回転次数候補に ∞ を追加（結晶は結晶学的制限で無し）。ただし直線分子の C∞ は全原子が軸上で動かない＝出題対象外のため、∞ は実質ディストラクター。サーバーは非整数次数を安全に不正解処理。
- **単位格子の等価原子表示**：`/api/render_data` に `?boundary_images=` 上書きを追加し、パズルは結晶で境界の周期像を表示（共有状態は汚さない）。`scope` 上書きと同じ方式。
- **UI**：右枠に元素ラベル＋投影＋カメラ操作、下段フル幅に問題。描画列は約1.3倍に拡幅。種類選択で次数欄が出ても画面がずれないよう `.puzzle-play` の横幅を固定＋次数欄の高さを予約。名称のみ表示（点群/空間群は答えになるため非表示）。

### 精査で見つけて直した不具合

- `/api/render_data` の `?boundary_images=` 追加時にインデントを崩し、`element_colors`/`element_hidden` 等が `if` 配下に入って**クエリ無しの通常リクエストが `UnboundLocalError` で500**になっていた。ロック内へ戻して修正（halite: 境界なし8原子／境界込み27原子を確認）。
...
当初は0問経路にも `puzzle-view-along` の非表示を足したが、実ブラウザで検証したところ**この指摘は成立しない**とわかったので、追加した行は撤回した。

`startStructure` は読み込み開始前（`puzzle.js:330`、`/api/open_example` を叩く前）に `puzzle-view-along` を隠す。ボタンを再表示するのは `beginRound()` の1箇所だけで、0問経路はそこへ到達せず return する。したがって前の構造のボタンが残ることはない。移り先で benzene を遊んでから HCl を選ぶ手順を Chromium で実際に踏み、修正前のコードでもボタンが消えることを確認した（後述のブラウザ検証、`test_structure_with_no_questions_leaves_no_dead_controls` は修正を戻しても pass する）。

再発防止として、`test_puzzle_client_contract.py` の該当テストを「0問経路に hidden 代入があること」ではなく**実際の保証**（読み込み前に隠す・再表示は `beginRound()` の1箇所だけ・0問経路はそこへ行かない）を固定する形に書き直した。
...
## 実ブラウザ検証（Claude 2026-08-08・Chromium + Playwright を導入）

前回の残課題「実ブラウザでの目視が未実施」を解消した。ユーザー了承のもと Chromium と Playwright をテスト用に導入し、**実際に遊んで**確認した。

### 依存関係の置き場所

パッケージには不要なので `requirements.txt` は触らず、`requirements-dev.txt` を新設した。
...
`tests/test_puzzle_browser.py` は playwright かブラウザバイナリが無ければモジュールごと skip する。したがって実行にもパッケージングにも Chromium は要らない。

### 追加したブラウザテスト（6件・約56秒）

`tools/view_json_server.py` を実際にサブプロセスで起動し、headless Chromium で人と同じ操作をする。

| テスト | 何を固定するか |
|---|---|
| `test_mapping_prompt_names_an_operation_the_server_published` | 画面に出る「操作 ○○ で…」が、サーバが公開したラベルのどれかであること。benzene が S3 と S6 を別ラベルで公開していること |
| `test_answering_adds_the_target_ring_to_the_source_and_guess_rings` | 起点1本 → クリックで2本 → 回答後3本。リングが増えるだけで消えないこと |
| `test_structure_with_no_questions_leaves_no_dead_controls` | HCl（0問）で「操作要素方向から見る」「回答」「再生」が消えていること |
| `test_composition_reveal_never_prints_two_conflicting_notations` | 合成の結果表示に `（S3）（S6）` のような自己矛盾が出ないこと（8ラウンド） |
| `test_answering_a_high_symmetry_crystal_stays_responsive` | halite で「回答クリック→画面に判定が出る」まで1秒未満（3ラウンド） |
| `test_no_uncaught_page_errors` | 全操作を通して未捕捉の JS 例外が0件 |

**全体で 222 tests OK**（216 + ブラウザ6件、約69秒）。

### 実機で確認できたこと

- **#1 の表記**: benzene の移り先40ラウンドで `C2 / C3 / C6 / S3 / S6 / i / σ` が出題され、レンダリングされたラベルはすべてサーバ公開分と一致。**S3 と S6 が別の問題として実際に画面に出た**。合成20ラウンドの結果表示は `正解（回映（S3））` `不正解（正解は 回映（S6））` で、二重表記は0件。
  - 修正を戻すと、同じテストが実機で **`不正解（正解は 回映（S3）（S6））`** を捕まえた。レビューが指摘した文字列そのものが再現する。
- **#2 の体感**: halite の合成で「回答クリック→判定表示」が **0.14〜0.17 秒**（レビュー時は 1.5 秒）。構造選択から遊べるまでは 4.0 秒（解析・レンダリング込み）。
- **#4 のリング**: 正解時に3本が同心になる場面をスクリーンショットで比較した。係数 `0.90`（修正前）では青と黄が隙間なく接して1本の帯に見え、`0.84`（修正後）では青・黄・緑の間にはっきり地が見える。**レビューの指摘どおりで、修正で解消**。
- **高速切替**: 回答/もう一度を10連打、別の物質を6連打しても、最後は1つの遊べるラウンドに収束し、その後も正常に回答できる。JS 例外0件。
- 回答ボタンを再生中に押すと「操作A・Bの再生が終わってから回答してください。」が出る（想定どおりの動作）。

### 判明した誤検出

- **#3 は成立しなかった**（上記のとおり）。実機で修正前のコードを動かして初めてわかった。追加した行は撤回済み。
