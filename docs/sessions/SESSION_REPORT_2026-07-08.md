...
- 難易度（普通／難しい）を追加。**難しい＝並進を含む操作（らせん・映進）のみ**（結晶専用）。
...
- 回転軸クイズを構造の複雑さで「やさしい／ふつう／むずかしい」に分け、構造ピッカーの候補を段階化。
- `docs/PUZZLE_SPEC.md`を実装済み仕様へ更新し、手動検証項目を`docs/PUZZLE_MANUAL_CHECKLIST.md`に整理。
...
- 共有サーバーセッションを汚さないため**クエリ上書き**を導入：
  ...
  - `/api/puzzle/operations?difficulty=`。
...
## 4. 難易度・要素表示・出題整理・軽量化（A/B/C/D、コミット済み）

- **A：回答後に対称要素を表示**。`puzzle.js: onCheckOperation()`で`view.loadSymmetryElements(op, gen)`を呼び、軸・面・中心・映進矢印を解析同様に表示。出題中は非表示。
- **B：難易度（普通／難しい）**。`identify_questions(render_data, difficulty)`で分岐。難しいは`_translation_answer`（らせん＝screw＋次数、映進＝glide）のみ。API/`check_answer`/`public_questions`もdifficultyを受ける。クイズ選択カードを3枚化、難しい選択時は結晶ピッカーへ。分子×難しいは出題なしメッセージ。
...
- 軸クイズを「やさしい／ふつう／むずかしい」の3カードに分けた。判定ロジックは変えず、構造ピッカーに出す候補を段階化する。
- やさしい: water, ammonia。軸が少なく、2回・3回を見分ける。
- ふつう: benzene, BF3, methane, XeF4, CO2など。C4/C6、複数軸、C∞を扱う。
- むずかしい: SF6と結晶全般。高対称・空間群由来の軸を扱う。
- 操作あてクイズの普通／難しいは従来どおり操作種による難易度として残す。
...
- 操作クイズ：CO2（反転・鏡映どちらも正解の1問）、methane（回映S4）、halite（回反-3/-4、難しいで51問・映進面表示）、∞選択の安全な不正解処理。
- 出題均等化：halite normal 137問→7 group、water 2 group。

## 次の方針

1. `docs/PUZZLE_MANUAL_CHECKLIST.md`に沿って代表構造を手動確認し、出題数・見やすさ・難易度分けを調整する。
